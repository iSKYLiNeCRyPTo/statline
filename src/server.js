...
// ── Persist xuid->gamertag cache to DB ───────────────────────────────────────
async function loadXuidCache() {
  try {
    const db = await getDb();
    if (!db) return;
    await db.query(`CREATE TABLE IF NOT EXISTS xuid_cache (xuid TEXT PRIMARY KEY, gamertag TEXT NOT NULL, ts TIMESTAMPTZ NOT NULL DEFAULT NOW())`);
    const result = await db.query('SELECT xuid, gamertag FROM xuid_cache');
    if (result.rows.length > 0) {
      const { getXuidToGt } = require('./halo');
      const gt = getXuidToGt();
      result.rows.forEach(r => { if (!gt[r.xuid]) gt[r.xuid] = r.gamertag; });
      console.log('[DB] Loaded', result.rows.length, 'cached xuids from DB');
    }
  } catch(e) { console.error('[DB] loadXuidCache error:', e.message); }
}
loadXuidCache();

// Periodically flush new xuid->gamertag entries to DB (every 5 min)
setInterval(async () => {
  try {
    const db = await getDb();
    if (!db) return;
    const { getXuidToGt } = require('./halo');
    const gt = getXuidToGt();
    const entries = Object.entries(gt).filter(([,v]) => v && !v.startsWith('Spartan '));
    if (!entries.length) return;
    // Upsert in batches of 100
    for (let i = 0; i < entries.length; i += 100) {
      const batch = entries.slice(i, i + 100);
      const values = batch.map((_, j) => `($${j*2+1},$${j*2+2})`).join(',');
      const params = batch.flatMap(([xuid, gamertag]) => [xuid, gamertag]);
      await db.query(
        `INSERT INTO xuid_cache (xuid, gamertag) VALUES ${values} ON CONFLICT (xuid) DO UPDATE SET gamertag=EXCLUDED.gamertag, ts=NOW()`,
        params
      );
    }
