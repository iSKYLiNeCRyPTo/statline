...
app.get('/api/admin/searches', async (req, res) => {
  ...
  try {
    ...
    if (db) {
      const result = await db.query('SELECT ts, gamertag, ip, user_agent, cached, success, duration_ms FROM search_log ORDER BY ts DESC LIMIT 500');
      searches = result.rows;
      const tabResult = await db.query(`
        SELECT tab, COUNT(*) as visits, ROUND(AVG(seconds),1) as avg_seconds, ROUND(SUM(seconds)/60,1) as total_minutes
        FROM tab_log GROUP BY tab ORDER BY visits DESC
      `).catch(() => ({ rows: [] }));
      tabStats = tabResult.rows;
    } else {
      searches = _memSearchLog.slice().reverse();
      tabStats = _memTabLog.reduce((acc, t) => {
        const e = acc.find(x => x.tab === t.tab);
        if (e) { e.visits++; e.total_seconds += t.seconds; e.avg_seconds = (e.total_seconds/e.visits).toFixed(1); }
        else acc.push({ tab: t.tab, visits: 1, avg_seconds: t.seconds.toFixed(1), total_minutes: (t.seconds/60).toFixed(1), total_seconds: t.seconds });
        return acc;
      }, []);
    }
    ...
    searches.forEach(s => {
      ...
      if (/mobile|android|iphone|ipad/.test(ua)) devices.mobile++;
...
app.get('/api/admin', (req, res) => {
  ...
  function loadData(){
    fetch('/api/admin/searches?pass=${pass}').then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json();}).then(d=>{
      ...
      document.getElementById('summary').innerHTML=
        ...
        '<div class="stat"><div class="stat-val">'+(dev.mobile||0)+'</div><div class="stat-lbl">MOBILE SEARCHES</div></div>'+
        '<div class="stat"><div class="stat-val">'+(dev.desktop||0)+'</div><div class="stat-lbl">DESKTOP SEARCHES</div></div>';
      var tabs=d.tabStats||[];
      maxVisits=tabs.reduce(function(m,t){return Math.max(m,parseInt(t.visits)||0);},1);
      document.getElementById('tabtbody').innerHTML=tabs.map(function(t){
        var pct=Math.round((parseInt(t.visits)/maxVisits)*100);
        return '<tr><td style="color:#00d4ff">'+t.tab+'</td>'+
          ...
          '<td class="muted">'+t.total_minutes+'m total</td></tr>';
      }).join('');
      renderRows(allRows);
  ...
  function renderRows(rows){
    document.getElementById('tbody').innerHTML=rows.map(function(s){
      ...
      var cachedStr=cached==='cached'?'<span class="muted">cached</span>':cached==='inflight'?'<span style="color:#555">inflight</span>':cached==='error'?'<span class="loss">error</span>':'<span style="color:#888">fresh</span>';
